...
> Click image for feature walkthrough video!

[![Feature Walkthrough Video](assets/brand/CyberSpy_Logo_Slogan_2.12.1.jpg)](https://player.vimeo.com/video/1187322703?loop=1)
...
- **Forbidden Phrases Enforcement**: Operator-managed forbidden phrases are enforced deterministically through the sanitizer. The visible Firewall Prompt also carries the recommended forbidden-category and gibberish/obfuscation guidance used by the safeguard judge; the backend appends only the structured JSON verdict contract outside that user-visible prompt.
...
- **Split Prompt Roles**: System configuration keeps a **Firewall Prompt** and a **Downstream Responder Prompt** distinct. The firewall prompt and guardrails policy are sent to the OpenAI-compatible safeguard judge for inspection and forwarding decisions, while the backend appends its JSON verdict contract outside the user-visible prompt. The downstream responder prompt is sent only after the safeguard judge returns `CLEAN`.
- **Visible Firewall Category Guidance**: The recommended Firewall Prompt includes the baseline forbidden-category and gibberish guidance operators review in System Configuration. Active forbidden phrases can still add runtime context for the safeguard judge without changing the backend-owned JSON verdict contract.
...
- **Layered Defense Metrics**: The Metrics view now calculates three explicit funnel rates for governed traffic: **Pre-Inference Block Rate** (% blocked before the Safeguard LLM), **Model Intervention Rate** (% caught by the Safeguard LLM after reaching it), and **Post-Model Escape Rate** (% of likely malicious prompts that bypass both layers and still land clean/informational). Backend safeguard outcomes are carried as structured audit/metric fields (`backendGatewayStatus`, `backendSafeguardVerdict`, `backendSafeguardReasoning`, `backendReachedSafeguard`) so Bulk Ingest and Analyst Chat interceptions are attributed to the correct layer instead of inferred from response text. When present, bulk-ingest `expectedVerdict` labels are used to strengthen escape-rate calculations.
...
- **Downstream Responder Tab**: Admin view exposes a dedicated responder runtime surface with provider, backend health, model/base URL, context-window configuration, responder key source, last forwarded prompt hash, responder latency, token usage, prompt profile, and output-sanitization signals. Operators can keep Analyst Chat unchanged while choosing an OpenAI-compatible or Gemini downstream responder for clean traffic. Responder API keys can remain backend-managed or be supplied as browser-memory-only overrides for demos, and the active **Downstream Responder Prompt** is sent as the responder instruction. Sam Spade CTF responder calls are identified with the `sam_spade_ctf` prompt profile without exposing the private scenario text in telemetry.
